...
for dist in appletvos-arm64/1300 iphoneos-arm64/1{5,6}00 watchos-arm64/1400 watchos-arm/1400; do
    ...
    apt-ftparchive packages pool/main/${dist}/ > \
                dists/${dist}/main/${binary}/Packages
    ...
    apt-ftparchive release -c config/${arch}-basic.conf dists/${dist}/main/${binary} > dists/${dist}/main/${binary}/Release
    apt-ftparchive release -c config/$(echo "${dist}" | cut -f1 -d '/').conf dists/${dist} > dists/${dist}/Release
